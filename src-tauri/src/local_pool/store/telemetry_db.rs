...
impl TelemetryDb {
    pub fn open(path: &Path) -> Result<Self> {
        ...
        if version > 1 {
            return Err(LocalPoolError::new(
                ...
                format!("usage database schema {version} is newer than supported schema 1"),
            ));
        }
        if version == 0 {
        ...
        self.connection
            ...
            .execute(
                "INSERT OR IGNORE INTO request_logs (
                    request_id, local_key_id, source_id, requested_model, resolved_model,
    ...
    pub fn list(&self, limit: u16) -> Result<Vec<UsageLog>> {
        ...
        let mut statement = connection
            .prepare(
                "SELECT id, strftime('%Y-%m-%dT%H:%M:%SZ', created_at), request_id, local_key_id, source_id, requested_model,
        ...
        let logs = statement
            .query_map([limit.clamp(1, 500)], |row| {
                ...
                let ttft_ms: Option<i64> = row.get(12)?;
                let input_tokens: Option<i64> = row.get(13)?;
                let output_tokens: Option<i64> = row.get(14)?;
                let total_tokens: Option<i64> = row.get(15)?;
                Ok(UsageLog {
                    id: row.get(0)?,
                    created_at: row.get(1)?,
                    request_id: row.get(2)?,
                    local_key_id: row.get(3)?,
                    source_id: row.get(4)?,
