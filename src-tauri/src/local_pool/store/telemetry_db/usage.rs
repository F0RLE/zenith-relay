...
pub(super) fn usage_model_equivalents(
    ...
    let rows = statement
        .query_map(params_from_iter(values.iter()), |row| {
            ...
            let cache_write_5m_tokens = row.get::<_, Option<i64>>(6)?.map(rust_u64);
            let cache_write_1h_tokens = row.get::<_, Option<i64>>(7)?.map(rust_u64);
            let unknown_cache_write_tokens = row.get::<_, Option<i64>>(8)?.map(rust_u64);
            let output_tokens = row.get::<_, Option<i64>>(9)?.map(rust_u64);
            let total_tokens = row.get::<_, Option<i64>>(10)?.map(rust_u64);
            let input_samples = rust_u64(row.get(11)?);
            let cached_samples = rust_u64(row.get(12)?);
            let cache_write_samples = rust_u64(row.get(13)?);
            let output_samples = rust_u64(row.get(14)?);
            let total_samples = rust_u64(row.get(15)?);
        ...
        .query_map(params_from_iter(parameters.iter()), |row| {
            ...
            Ok((
                ...
                estimate_candidate_equivalent(
                    ...
                    input_tokens,
                    cached_input_tokens,
                    cache_writes.map(|_| cache_write_5m_tokens.map(rust_u64).unwrap_or_default()),
                    cache_writes.map(|_| cache_write_1h_tokens.map(rust_u64).unwrap_or_default()),
                    cache_writes
                        .map(|_| unknown_cache_write_tokens.map(rust_u64).unwrap_or_default()),
                    (output_samples > 0)
