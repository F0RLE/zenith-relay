...
pub(super) const MAX_JWT_BYTES: usize = 64 * 1024;

pub(super) const MAX_JWT_PAYLOAD_BYTES: usize = 16 * 1024;
...
#[derive(Default)]
pub(super) struct ImportedIdentity {
    pub(super) email: Option<String>,
    ...
    pub(super) access_expires_at_ms: Option<u64>,
}
...
pub(super) fn masked_account_identity(value: &str) -> String {
    let suffix = value
        .chars()
        .rev()
...
pub(super) fn timestamp_from_ms(value: u64) -> Option<String> {
...
pub(super) async fn import_source_item(
    state: &DesktopState,
    item: ParsedImportItem,
    add_to_pool: bool,
...
pub(super) fn imported_source_record(
    item: &ParsedImportItem,
...
pub(super) fn imported_source_base_url(item: &ParsedImportItem) -> ItemResult<String> {
...
pub(super) fn imported_source_wire_api(
    item: &ParsedImportItem,
    existing: Option<&ProviderSourceRecord>,
) -> ItemResult<WireApi> {
...
pub(super) fn canonical_source_base_url(value: &str) -> ItemResult<String> {
...
pub(super) fn source_identity_key(base_url: &str, api_key: &str) -> ItemResult<String> {
...
pub(super) fn find_existing_source(
    state: &DesktopState,
    base_url: &str,
    api_key: &str,
...
pub(super) async fn persist_imported_source(
    state: &DesktopState,
...
pub(super) fn current_source_records(
    state: &DesktopState,
...
pub(super) fn restore_source_secret(secret_ref: &str, previous: Option<&str>) -> ItemResult<()> {
    match previous {
...
pub(super) fn account_id_from_check_response(payload: &serde_json::Value) -> Option<String> {
...
pub(super) fn account_id_from_profile_record(record: &serde_json::Value) -> Option<String> {
    let record = record
        .get("account")
...
pub(super) fn normalized_profile_account_id(value: &str) -> Option<String> {
...
pub(super) fn provider_identity_key(
...
pub(super) fn parse_subscription_timestamp_ms(value: &str) -> Option<u64> {
    let value = value.trim();
