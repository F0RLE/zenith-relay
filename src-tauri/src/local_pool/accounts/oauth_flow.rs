...
mod tests {
    ...
    async fn loopback_callback_validates_state_and_stores_only_secret_material() {
        ...
        assert!(response.contains("automatically in 10 seconds"));
        assert!(response.contains("window.setTimeout(() => window.close(), 10000)"));
