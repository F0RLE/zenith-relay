...
impl GatewayRuntime {
    ...
    /// management surface consults confirmed metadata for an eligible route,
    /// so an unavailable account, cooling route, or generic API spelling
    /// cannot fabricate a Fast control.
    ...
    /// A model can be shared by several routes. A confirmed Fast tier on one
    /// route must not authorize a priority request on another route that did
    /// not advertise it.
