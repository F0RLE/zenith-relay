...
pub fn subscription_refresh_due(
    ...
    if active_until_ms.is_some_and(|active_until_ms| active_until_ms > now_ms) {
        return false;
    }
    updated_at_ms
        .map(|updated_at_ms| {
...
pub fn parse_subscription_timestamp_ms(value: &Value) -> Option<u64> {
    match value {
        Value::Number(value) => value.as_u64().and_then(normalize_epoch_ms),
        Value::String(value) => {
            let value = value.trim();
...
mod tests {
    use super::*;
    use axum::{
        extract::Request, http::HeaderMap, response::IntoResponse, routing::get, Json, Router,
    };
    use serde_json::json;
