...
function KeysView({ onCreate }: { onCreate: () => void }) {
  ...
  return <><div className="relay-table-wrap"><table className="relay-table"><thead><tr><th>{t("common.status")}</th><th>{t("common.name")}</th><th>{t("keys.masked")}</th><th>{t("keys.scope")}</th><th>{t("common.models")}</th><th>{t("common.lastUsed")}</th><th><span className="sr-only">{t("common.actions")}</span></th></tr></thead><tbody>{runtime.keys.map((key) => <tr key={key.id}><td><StatusBadge status={key.enabled ? "ready" : "disabled"} label={key.enabled ? t("common.enabled") : t("common.disabled")} /></td><td>{key.label}</td><td><code>zlr_••••••••••••</code></td><td>{(key.accountIds?.length || key.sourceIds?.length) ? t("keys.scoped") : t("keys.allMembers")}</td><td>{key.allowedModels.length || t("keys.allModels")}</td><td>{key.lastUsedAtMs ? new Date(key.lastUsedAtMs).toLocaleString() : t("common.never")}</td><td className="row-actions"><IconButton label={t("keys.editPolicy")} icon={<Pencil aria-hidden />} onClick={() => setEditing(key)} /><IconButton label={key.enabled ? t("common.disable") : t("common.enable")} icon={<Power aria-hidden />} onClick={() => perform(`enable-${key.id}`, () => mode === "local" ? relayCommands.setKeyEnabled(key.id, !key.enabled) : relayCommands.remoteAction({ type: "update_key", id: key.id }, { enabled: !key.enabled }), "feedback.saved")} /><IconButton label={t("keys.rotate")} icon={<RotateCcw aria-hidden />} disabled={busy === `rotate-${key.id}`} onClick={async () => { if (!window.confirm(t("keys.rotateConfirm"))) return; const result: { current: { secret: string } | null } = { current: null }; await perform(`rotate-${key.id}`, async () => { result.current = mode === "local" ? await relayCommands.rotateKey(key.id) : await relayCommands.remoteAction({ type: "rotate_key", id: key.id }) as { secret: string }; }, "feedback.keyRotated"); if (result.current) setRevealed(result.current.secret); }} /><IconButton label={t("common.delete")} icon={<Trash2 aria-hidden />} onClick={() => { if (window.confirm(t("keys.deleteConfirm"))) void perform(`delete-${key.id}`, () => mode === "local" ? relayCommands.deleteKey(key.id) : relayCommands.remoteAction({ type: "delete_key", id: key.id }), "feedback.deleted"); }} /></td></tr>)}</tbody></table></div>{revealed ? <div className="one-time-secret" role="status"><strong>{t("keys.copyNow")}</strong><code>{revealed}</code><Button variant="secondary" onClick={() => navigator.clipboard.writeText(revealed)}>{t("common.copy")}</Button></div> : null}{editing ? <KeyPolicyDialog key={editing.id} value={editing} onClose={() => setEditing(null)} /> : null}</>;
}
