...
export function PoolPage() {
  ...
  const { runtime, setPage } = useRelayState();
  const [view, setView] = useState<View>("members");
  const [createKey, setCreateKey] = useState(false);
  const action = view === "keys"
    ? <Button variant="primary" icon={<KeyRound aria-hidden />} onClick={() => setCreateKey(true)}>{t("keys.create")}</Button>
    : view === "members"
      ? <Button variant="primary" icon={<Plus aria-hidden />} onClick={() => setPage("connections")}>{t("pool.addMember")}</Button>
      : null;
  return <section className="relay-page"><PageHeader title={t("nav.pool")} subtitle={t("pool.subtitle")} actions={action} /><Tabs value={view} onChange={(id) => setView(id as View)} label={t("pool.views")} items={[{ id: "members", label: t("pool.members") }, { id: "keys", label: t("pool.keys") }, { id: "models", label: t("pool.modelRules") }]} />{view === "members" ? <MembersView /> : null}{view === "keys" ? <KeysView onCreate={() => setCreateKey(true)} /> : null}{view === "models" ? <ModelsView /> : null}{createKey ? <CreateKeyDialog onClose={() => setCreateKey(false)} /> : null}{!runtime ? <span className="sr-only">{t("common.notConfigured")}</span> : null}</section>;
}
...
function MembersView() {
  ...
  if (!members.length) return <EmptyState title={t("pool.emptyTitle")} description={t("pool.emptyDescription")} action={<Button variant="primary" onClick={() => setPage("connections")}>{t("pool.addMember")}</Button>} />;
...
function KeysView({ onCreate }: { onCreate: () => void }) {
  ...
  if (!runtime?.keys.length) return <EmptyState title={t("keys.emptyTitle")} description={t("keys.emptyDescription")} action={<Button variant="primary" onClick={onCreate}>{t("keys.create")}</Button>} />;
