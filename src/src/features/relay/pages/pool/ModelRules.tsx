...
import { BrainCircuit, CircleAlert, GripVertical, Loader2, Pencil, Power, RotateCcw, Zap } from "lucide-react";
import { useTranslation } from "react-i18next";
import { relayCommands } from "../../api/commands";
import type { ModelSummary } from "../../api/types";
import { Button, Dialog, EmptyState, IconButton, OptionMenu, StatusIcon } from "../../components/Ui";
...
export function ModelRulesView() {
  ...
  return <>{discoveryAlert}<section className="model-rules relay-compact-content">
    ...
    <div className="relay-table-wrap"><table className="relay-table model-rules-table">
      <colgroup><col data-column="model" /><col data-column="codex" /><col data-column="price" /><col data-column="members" /><col data-column="speed" /><col data-column="actions" /></colgroup>
      <thead><tr><th>{t("common.model")}</th><th>{t("models.codexColumn")}</th><th>{t("models.priceColumn")}</th><th>{t("pool.members")}</th><th>{t("models.speedColumn")}</th><th>{t("common.actions")}</th></tr></thead>
      {modelGroups.map((group) => <tbody key={group.id}>
      <tr className={`model-group-row${dragGroupId === group.id ? " model-dragging" : ""}`} draggable onDragStart={(event) => startGroupDrag(event, group.id)} onDragEnd={() => setDragGroupId(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragGroupId) reorderGroups(dragGroupId, group.id); setDragGroupId(null); }}><th colSpan={6} scope="rowgroup"><GripVertical aria-hidden /><strong>{t(`modelGroups.${group.id}`, { defaultValue: group.label })}</strong><span>{t("models.groupCount", { count: group.items.length })}</span></th></tr>
      ...
      return <tr key={model.id} data-model-id={model.id} data-enabled={model.enabled ? "true" : "false"} className={dragModelId === model.id ? "model-dragging" : undefined} draggable onDragStart={(event) => startModelDrag(event, model.id)} onDragEnd={() => setDragModelId(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragModelId) reorderModels(dragModelId, model.id); setDragModelId(null); }}>
