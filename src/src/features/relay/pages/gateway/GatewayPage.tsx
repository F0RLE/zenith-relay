...
import { Button, Dialog, EmptyState, OptionMenu, PageHeader, SecretField, StatusBadge, Tabs, copyText, formatAccountPlan, isCodexOauthAccountEligible } from "../../components/Ui";
...
function EndpointView({ running, endpoint }: { running: boolean; endpoint: string }) {
  ...
  return <div className="gateway-sections">
    <section className="gateway-runtime-panel"><h2 className="sr-only">{t("gateway.runtime")}</h2><div className="endpoint-line"><code>{endpoint}</code><StatusBadge status={running ? "ready" : "warning"} label={running ? t("common.online") : t("common.offline")} /><Button variant="secondary" icon={<Copy aria-hidden />} onClick={() => copyText(endpoint)}>{t("common.copy")}</Button></div><dl className="runtime-grid"><div><dt>{t("common.status")}</dt><dd>{running ? t("common.online") : t("common.offline")}</dd></div><div><dt>{t("common.models")}</dt><dd>{runtime?.gateway.visibleModelIds.length ?? "-"}</dd></div><div><dt>{t("pool.members")}</dt><dd>{runtime?.gateway.candidateCount ?? "-"}</dd></div><div><dt>{t("common.mode")}</dt><dd>{t(`modes.${mode}`)}</dd></div></dl></section>
    <section><h2>{t("gateway.settings")}</h2><div className={`settings-row${mode === "local" ? " gateway-address-settings" : ""}`}><label><span>{t("gateway.host")}</span><input value={mode === "local" ? "127.0.0.1" : new URL(endpoint).host} readOnly /></label><label><span>{t("gateway.port")}</span><input type="number" min="1024" max="65535" value={port} disabled={mode !== "local"} onChange={(event) => setPort(event.target.value)} /></label><label><span>{t("gateway.bind")}</span><input value={mode === "local" ? t("gateway.loopback") : t("gateway.serverManaged")} readOnly /></label>{mode === "local" ? <Button variant="secondary" busy={busy === "gateway-port"} disabled={!portValid || port === currentPort} icon={<Save aria-hidden />} onClick={() => perform("gateway-port", () => relayCommands.updateGatewayPort(numericPort), "feedback.saved")}>{running ? t("gateway.applyRestart") : t("common.save")}</Button> : null}</div></section>
