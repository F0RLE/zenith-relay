import { Activity, ArrowRight, CircleAlert, Play, Server, Square, Users } from "lucide-react";
  ...
  return <section className="relay-page"><PageHeader title={t("nav.overview")} subtitle={t(`overview.subtitles.${mode}`)} actions={primary} />
    {!running && !runtime && mode !== "zenith" ? <EmptyState title={t("overview.emptyTitle")} description={t("overview.emptyDescription")} action={<Button variant="primary" onClick={() => setPage("connections")}>{t("overview.openConnections")}</Button>} /> : <>
      <div className="overview-summary">
        <div className="overview-status-band"><StatusBadge status={running ? "ready" : "warning"} label={running ? t("common.ready") : t("common.offline")} /><div><span>{t("common.mode")}</span><strong>{t(`modes.${mode}`)}</strong></div><div><span>{mode === "zenith" ? t("readyApi.balance") : t("pool.capacity")}</span><strong>{mode === "zenith" ? readyStats?.balance ?? "-" : t("pool.membersCount", { count: runtime?.gateway.candidateCount ?? 0 })}</strong></div></div>
