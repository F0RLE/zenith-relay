...
import { FolderOpen, RefreshCw, RotateCcw, ShieldCheck, Trash2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { updateAndRelaunch } from "../../../../tauri";
import { Button, PageHeader, StatusBadge } from "../../components/Ui";
import { useRelayState } from "../../state/RelayStateProvider";

const sections = ["general","appearance","storage","updates","security","recovery"] as const;

export function SettingsPage() {
  const { t, i18n } = useTranslation(); const { theme, setTheme, compact, setCompact, mode, resetOnboarding, perform, busy } = useRelayState(); const [section, setSection] = useState<(typeof sections)[number]>("general");
  return <section className="relay-page"><PageHeader title={t("nav.settings")} subtitle={t("settings.subtitle")} /><div className="settings-layout"><nav aria-label={t("settings.sections")}>{sections.map((item) => <button key={item} className={section === item ? "active" : ""} type="button" onClick={() => setSection(item)}>{t(`settings.${item}`)}</button>)}</nav><div className="settings-content">
    ...
    {section === "storage" ? <><h2>{t("settings.storage")}</h2><dl className="detail-list"><div><dt>{t("settings.dataPath")}</dt><dd><code>{t("settings.platformDataPath")}</code></dd></div><div><dt>{t("settings.retention")}</dt><dd>30 {t("settings.days")}</dd></div></dl><Button variant="secondary" icon={<FolderOpen aria-hidden />} disabled>{t("settings.openData")}</Button></> : null}
    ...
    {section === "recovery" ? <><h2>{t("settings.recovery")}</h2><p>{t("settings.recoveryHint")}</p><Button variant="secondary" icon={<RotateCcw aria-hidden />}>{t("settings.restoreBackup")}</Button><div className="danger-zone"><h3>{t("settings.resetData")}</h3><p>{t("settings.resetDataHint")}</p><Button variant="danger" icon={<Trash2 aria-hidden />} disabled>{t("settings.resetData")}</Button></div></> : null}
  </div></div></section>;
}
