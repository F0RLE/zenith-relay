...
import { CalendarDays, Check, ChevronDown, Clock3, Copy, CreditCard, Download, ExternalLink, Eye, EyeOff, Layers3, ListMinus, ListPlus, Loader2, LogIn, Network, Pencil, Play, Plus, Power, RefreshCw, ShieldAlert, SlidersHorizontal, Trash2, Unlink, Upload, UserRoundX, X } from "lucide-react";
...
export function ConnectionsPage({ onImport }: { onImport: () => void }) {
  ...
  return (
    <section className="relay-page" data-view={view}>
      ...
      {view === "api" ? <ReadyApiView connected={Boolean(readyState?.providerActive)} onConnect={() => setDialog("ready")} /> : null}
...
function ReadyApiView({ connected, onConnect }: { connected: boolean; onConnect: () => void }) {
  const { t } = useTranslation();
  const { readyStats, perform } = useRelayState();
  const confirm = useConfirm();
  if (!connected) return <section className="ready-api-connection"><div className="ready-api-empty"><Network aria-hidden /><div><h2>{t("readyApi.emptyTitle")}</h2><p>{t("readyApi.emptyDescription")}</p></div></div><ApiProtocolGuide /></section>;
  return <section className="ready-api-connection"><div className="ready-api-connected-heading"><div><strong>Zenith API</strong><small>https://api.zenithmarket.dev/v1</small></div><StatusBadge status="ready" label={t("common.connected")} /></div><dl className="detail-list"><div><dt>{t("readyApi.balance")}</dt><dd>{readyStats?.balance ?? "-"}</dd></div><div><dt>{t("usage.requests")}</dt><dd>{readyStats?.requestsDisplay ?? readyStats?.requests ?? "-"}</dd></div></dl><div className="inline-actions"><Button variant="secondary" onClick={onConnect}>{t("readyApi.updateKey")}</Button><Button variant="danger" onClick={() => void confirm(t("readyApi.disconnectConfirm"), { danger: true }).then((accepted) => accepted && perform("ready-disconnect", resetKey, "feedback.disconnected"))}>{t("remote.disconnect")}</Button></div></section>;
}

function ApiProtocolGuide() {
  const { t } = useTranslation();
  return <section className="api-protocol-guide"><h3>{t("apiProviders.protocols.title")}</h3><div><code>/v1/responses</code><span><strong>{t("apiProviders.protocols.responses.label")}</strong><small>{t("apiProviders.protocols.responses.hint")}</small></span></div><div><code>/v1/chat/completions</code><span><strong>{t("apiProviders.protocols.chatCompletions.label")}</strong><small>{t("apiProviders.protocols.chatCompletions.hint")}</small></span></div></section>;
}
...
export function SourceDialog({ source, onClose, addToPool = false }: { source: SourceSummary | null; onClose: () => void; addToPool?: boolean }) {
  ...
  const protocol = wireApi === "responses" ? "responses" : "chatCompletions";
  ...
  return <Dialog wide title={source ? t("sources.edit") : addToPool ? t("sources.addToPool") : t("sources.add")} onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>{t("common.cancel")}</Button><Button variant="primary" busy={busy === "source-save"} disabled={!source && !apiKey.trim()} onClick={() => document.querySelector<HTMLFormElement>("#source-form")?.requestSubmit()}>{t("common.save")}</Button></>}><form id="source-form" className="relay-form source-form" onSubmit={submit}><section className="source-form-section"><header><h3>{t("sources.connection")}</h3><p>{t("sources.connectionHint")}</p></header><div className="source-identity-grid"><label className="relay-field"><span>{t("common.name")}</span><input value={name} onChange={(event) => setName(event.target.value)} required /></label><label className="relay-field"><span>{t("sources.address")}</span><input type="url" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} placeholder="https://api.example.com/v1" required /></label></div><div className="source-access-grid"><div className="relay-field"><span>{t("sources.protocol")}</span><OptionMenu className="field-option-menu" label={t("sources.protocol")} value={wireApi} onChange={(value) => setWireApi(value as SourceSummary["wireApi"])} options={[{ value: "responses", label: "Responses API" }, { value: "chat_completions", label: "Chat Completions" }]} /></div><SecretField label={source ? t("sources.replaceKey") : t("sources.apiKey")} value={apiKey} onChange={setApiKey} /></div><div className="api-protocol-help"><strong>{t(`apiProviders.protocols.${protocol}.label`)}</strong><span>{t(`apiProviders.protocols.${protocol}.hint`)}</span></div></section><details className="source-advanced"><summary><SlidersHorizontal aria-hidden /><span><strong>{t("sources.advanced")}</strong><small>{t("sources.advancedHint")}</small></span><ChevronDown aria-hidden /></summary><div><label className="relay-field"><span>{t("common.models")}</span><input value={models} onChange={(event) => setModels(event.target.value)} placeholder={t("sources.modelListPlaceholder")} /></label><p className="form-note">{t("sources.modelsHint")}</p><div className="settings-row"><label><span>{t("pool.allowedModels")}</span><input value={allowed} onChange={(event) => setAllowed(event.target.value)} /></label><label><span>{t("pool.excludedModels")}</span><input value={excluded} onChange={(event) => setExcluded(event.target.value)} /></label></div><div className="settings-row"><div className="relay-field"><span>{t("sources.poolRole")}</span><OptionMenu className="field-option-menu" label={t("sources.poolRole")} value={role} onChange={(value) => setRole(value as ApiSourceRole)} options={[{ value: "primary", label: t("sources.roles.primary") }, { value: "stabilizer", label: t("sources.roles.stabilizer") }, { value: "reserve", label: t("sources.roles.reserve") }]} /><small>{t(`sources.roleHints.${role}`)}</small></div><label><span>{t("pool.trafficShare")}</span><input type="number" min="1" value={weight} onChange={(event) => setWeight(Number(event.target.value))} /></label></div></div></details></form></Dialog>;
}
