...
import { ActionMenu, ActionMenuItem, Button, Dialog, EmptyState, PageHeader, StatusBadge, Tabs } from "../../components/Ui";
...
export function UsagePage() {
  ...
  const rows = useMemo<UsageRow[]>(() => {
    if (mode === "zenith") return readyUsage.map((item) => ({ id: item.id, time: item.createdAt, success: item.status === "success", model: item.modelDisplay || item.model, connection: "Zenith API", key: "Zenith API", wireApi: null, latency: item.streamDurationMs ?? item.timeToFirstByteMs ?? 0, inputTokens: item.inputTokens, cachedInputTokens: item.cachedInputTokens, reasoningTokens: item.reasoningTokens, outputTokens: item.outputTokens, tokens: item.totalTokens, requestId: item.requestId, httpStatus: item.status === "success" ? 200 : null, errorCategory: item.status === "success" ? null : item.status }));
    if (mode === "remote") return remoteUsage.map((item) => ({ id: item.id, time: new Date(item.createdAtMs).toISOString(), success: item.success, model: item.resolvedModel ?? item.requestedModel, connection: item.candidateLabel ?? item.candidateHint, key: item.localKeyId, wireApi: item.wireApi, latency: item.latencyMs, inputTokens: item.inputTokens, cachedInputTokens: item.cachedInputTokens, reasoningTokens: item.reasoningTokens, outputTokens: item.outputTokens, tokens: item.totalTokens, requestId: item.requestId, httpStatus: item.httpStatus, errorCategory: item.errorCategory }));
  ...
  return <section className="relay-page">
    ...
    <div className="metric-band usage-metrics"><div><span>{t("usage.requests")}</span><strong>{mode === "remote" ? remoteUsagePage?.total ?? 0 : rows.length}</strong></div><div><span>{t("common.success")}</span><strong>{success}</strong></div><div><span>{t("usage.totalTokens")}</span><strong>{rows.reduce((sum, item) => sum + (item.tokens ?? 0), 0)}</strong></div><div><span>{t("usage.latency")}</span><strong>{rows.length ? `${Math.round(rows.reduce((sum, item) => sum + item.latency, 0) / rows.length)} ms` : "-"}</strong></div></div>
...
function RequestsView({ rows, status, setStatus, range, setRange, modelQuery, setModelQuery, connectionQuery, setConnectionQuery, keyQuery, setKeyQuery, wireApi, setWireApi, errorQuery, setErrorQuery, requestQuery, setRequestQuery, formatTime, onSelect }: { rows: UsageRow[]; status: string; setStatus: (value: string) => void; range: Range; setRange: (value: Range) => void; modelQuery: string; setModelQuery: (value: string) => void; connectionQuery: string; setConnectionQuery: (value: string) => void; keyQuery: string; setKeyQuery: (value: string) => void; wireApi: string; setWireApi: (value: string) => void; errorQuery: string; setErrorQuery: (value: string) => void; requestQuery: string; setRequestQuery: (value: string) => void; formatTime: (value: string) => string; onSelect: (row: UsageRow) => void }) {
  ...
  return <><div className="table-toolbar usage-filters"><select value={range} onChange={(event) => setRange(event.target.value as Range)} aria-label={t("usage.range")}><option value="daily">{t("usage.daily")}</option><option value="weekly">{t("usage.weekly")}</option><option value="monthly">{t("usage.monthly")}</option><option value="all">{t("common.all")}</option></select><select value={status} onChange={(event) => setStatus(event.target.value)} aria-label={t("common.status")}><option value="all">{t("usage.anyStatus")}</option><option value="success">{t("common.success")}</option><option value="failed">{t("common.failed")}</option></select><input value={modelQuery} onChange={(event) => setModelQuery(event.target.value)} aria-label={t("common.model")} placeholder={t("common.model")} /><input value={connectionQuery} onChange={(event) => setConnectionQuery(event.target.value)} aria-label={t("usage.connection")} placeholder={t("usage.connection")} /><input value={keyQuery} onChange={(event) => setKeyQuery(event.target.value)} aria-label={t("usage.localKey")} placeholder={t("usage.localKey")} /><select value={wireApi} onChange={(event) => setWireApi(event.target.value)} aria-label={t("usage.protocol")}><option value="">{t("usage.anyProtocol")}</option><option value="responses">Responses</option><option value="chat_completions">Chat Completions</option></select><input value={errorQuery} onChange={(event) => setErrorQuery(event.target.value)} aria-label={t("usage.errorCategory")} placeholder={t("usage.errorCategory")} /><input value={requestQuery} onChange={(event) => setRequestQuery(event.target.value)} aria-label={t("usage.requestId")} placeholder={t("usage.requestId")} /></div>{rows.length ? <div className="relay-table-wrap"><table className="relay-table usage-request-table"><thead><tr><th>{t("usage.time")}</th><th>{t("common.status")}</th><th>{t("common.model")}</th><th>{t("nav.connections")}</th><th>{t("usage.latency")}</th><th>{t("usage.tokens")}</th><th>{t("usage.requestId")}</th></tr></thead><tbody>{rows.map((item) => <tr key={item.id}><td>{formatTime(item.time)}</td><td><StatusBadge status={item.success ? "ready" : "error"} label={item.success ? t("common.success") : t("common.failed")} /></td><td><code>{item.model ?? "-"}</code></td><td>{item.connection}</td><td>{item.latency} ms</td><td>{item.tokens ?? "-"}</td><td><button type="button" className="request-link request-disclosure" aria-haspopup="dialog" aria-label={`${t("usage.requestDetails")}: ${item.requestId ?? "-"}`} onClick={() => onSelect(item)}><code>{item.requestId ?? "-"}</code><ChevronRight aria-hidden /></button></td></tr>)}</tbody></table></div> : <EmptyState title={t("common.noResults")} description={t("common.noResultsHint")} />}</>;
}

function AggregateView({ rows, field, empty }: { rows: UsageRow[]; field: "model" | "connection"; empty: string }) {
  const { t } = useTranslation();
  const groups = [...rows.reduce((map, row) => { const key = row[field] || t("common.unknown"); const current = map.get(key) ?? { name: key, requests: 0, success: 0, inputTokens: 0, cachedInputTokens: 0, reasoningTokens: 0, outputTokens: 0, tokens: 0, latency: 0 }; current.requests += 1; current.success += Number(row.success); current.inputTokens += row.inputTokens ?? 0; current.cachedInputTokens += row.cachedInputTokens ?? 0; current.reasoningTokens += row.reasoningTokens ?? 0; current.outputTokens += row.outputTokens ?? 0; current.tokens += row.tokens ?? 0; current.latency += row.latency; map.set(key, current); return map; }, new Map<string, { name: string; requests: number; success: number; inputTokens: number; cachedInputTokens: number; reasoningTokens: number; outputTokens: number; tokens: number; latency: number }>()).values()];
  if (!groups.length) return <EmptyState title={t("usage.emptyTitle")} description={empty} />;
  return <div className="relay-table-wrap"><table className="relay-table usage-aggregate-table"><thead><tr><th>{field === "model" ? t("common.model") : t("usage.connection")}</th><th>{t("usage.requests")}</th><th>{t("common.success")}</th><th>{t("usage.inputTokens")}</th><th>{t("usage.cachedInputTokens")}</th><th>{t("usage.reasoningTokens")}</th><th>{t("usage.outputTokens")}</th><th>{t("usage.totalTokens")}</th><th>{t("usage.latency")}</th></tr></thead><tbody>{groups.map((group) => <tr key={group.name}><td><code>{group.name}</code></td><td>{group.requests}</td><td>{Math.round(group.success / group.requests * 100)}%</td><td>{group.inputTokens}</td><td>{group.cachedInputTokens}</td><td>{group.reasoningTokens}</td><td>{group.outputTokens}</td><td>{group.tokens}</td><td>{Math.round(group.latency / group.requests)} ms</td></tr>)}</tbody></table></div>;
}
...
function RequestDetails({ row, onClose }: { row: UsageRow; onClose: () => void }) {
  const { t } = useTranslation();
  return <Dialog title={t("usage.requestDetails")} onClose={onClose} footer={<Button variant="primary" onClick={onClose}>{t("common.close")}</Button>}><dl className="detail-list"><div><dt>{t("usage.requestId")}</dt><dd><code>{row.requestId ?? "-"}</code></dd></div><div><dt>{t("common.status")}</dt><dd>{row.success ? t("common.success") : t("common.failed")}</dd></div><div><dt>{t("common.model")}</dt><dd><code>{row.model ?? "-"}</code></dd></div><div><dt>{t("usage.connection")}</dt><dd>{row.connection}</dd></div><div><dt>{t("usage.httpStatus")}</dt><dd>{row.httpStatus ?? "-"}</dd></div><div><dt>{t("usage.errorCategory")}</dt><dd>{row.errorCategory ?? "-"}</dd></div><div><dt>{t("usage.latency")}</dt><dd>{row.latency} ms</dd></div><div><dt>{t("usage.inputTokens")}</dt><dd>{row.inputTokens ?? "-"}</dd></div><div><dt>{t("usage.cachedInputTokens")}</dt><dd>{row.cachedInputTokens ?? "-"}</dd></div><div><dt>{t("usage.reasoningTokens")}</dt><dd>{row.reasoningTokens ?? "-"}</dd></div><div><dt>{t("usage.outputTokens")}</dt><dd>{row.outputTokens ?? "-"}</dd></div><div><dt>{t("usage.totalTokens")}</dt><dd>{row.tokens ?? "-"}</dd></div></dl><p className="form-note">{t("usage.redactionHint")}</p></Dialog>;
}
