...
const USAGE_SUMMARY_METRICS = ["requests", "success", "tokens", "equivalent", "streamSpeed"] as const;
...
export function UsagePage() {
  ...
  const averageStreamSpeed = totals.speedDurationMs ? totals.speedOutputTokens * 1_000 / totals.speedDurationMs : null;
  ...
  return <section className="relay-page">
    ...
    {USAGE_SUMMARY_METRICS.some((metric) => summaryMetrics[metric]) ? <section className="usage-overview" aria-label={t("usage.summary")}>
      ...
      {summaryMetrics.streamSpeed ? <div className="usage-performance" data-separated={summaryMetrics.requests || summaryMetrics.success || summaryMetrics.tokens || summaryMetrics.equivalent ? "true" : undefined}>
        <UsageMetric label={t("usage.streamSpeed")} value={formatTokenSpeed(averageStreamSpeed, i18n.resolvedLanguage ?? i18n.language, speedUnit)} title={t("usage.streamSpeedHint")} />
      </div> : null}
    </section> : null}
