...
export function UsagePage() {
  ...
  const rows = useMemo<UsageRow[]>(() => {
    if (mode === "zenith") return readyUsage.map((item) => ({ id: item.id, time: item.createdAt, success: item.status === "success", model: item.modelDisplay || item.model, connection: "Zenith API", key: "Zenith API", wireApi: null, latency: item.streamDurationMs ?? item.timeToFirstByteMs ?? 0, inputTokens: item.inputTokens, cachedInputTokens: item.cachedInputTokens, outputTokens: item.outputTokens, tokens: item.totalTokens, requestId: item.requestId, httpStatus: item.status === "success" ? 200 : null, errorCategory: item.status === "success" ? null : item.status }));
    if (mode === "remote") return remoteUsage.map((item) => ({ id: item.id, time: new Date(item.createdAtMs).toISOString(), success: item.success, model: item.resolvedModel ?? item.requestedModel, connection: item.candidateLabel ?? item.candidateHint, key: item.localKeyId, wireApi: item.wireApi, latency: item.latencyMs, inputTokens: item.inputTokens, cachedInputTokens: item.cachedInputTokens, outputTokens: item.outputTokens, tokens: item.totalTokens, requestId: item.requestId, httpStatus: item.httpStatus, errorCategory: item.errorCategory }));
...
function AggregateView({ rows, field, empty }: { rows: UsageRow[]; field: "model" | "connection"; empty: string }) {
  const { t } = useTranslation();
  const groups = [...rows.reduce((map, row) => { const key = row[field] || t("common.unknown"); const current = map.get(key) ?? { name: key, requests: 0, success: 0, inputTokens: 0, cachedInputTokens: 0, outputTokens: 0, tokens: 0, latency: 0 }; current.requests += 1; current.success += Number(row.success); current.inputTokens += row.inputTokens ?? 0; current.cachedInputTokens += row.cachedInputTokens ?? 0; current.outputTokens += row.outputTokens ?? 0; current.tokens += row.tokens ?? 0; current.latency += row.latency; map.set(key, current); return map; }, new Map<string, { name: string; requests: number; success: number; inputTokens: number; cachedInputTokens: number; outputTokens: number; tokens: number; latency: number }>()).values()];
  if (!groups.length) return <EmptyState title={t("usage.emptyTitle")} description={empty} />;
  return <div className="relay-table-wrap"><table className="relay-table usage-aggregate-table"><thead><tr><th>{field === "model" ? t("common.model") : t("usage.connection")}</th><th>{t("usage.requests")}</th><th>{t("common.success")}</th><th>{t("usage.inputTokens")}</th><th>{t("usage.cachedInputTokens")}</th><th>{t("usage.outputTokens")}</th><th>{t("usage.totalTokens")}</th><th>{t("usage.latency")}</th></tr></thead><tbody>{groups.map((group) => <tr key={group.name}><td><code>{group.name}</code></td><td>{group.requests}</td><td>{Math.round(group.success / group.requests * 100)}%</td><td>{group.inputTokens}</td><td>{group.cachedInputTokens}</td><td>{group.outputTokens}</td><td>{group.tokens}</td><td>{Math.round(group.latency / group.requests)} ms</td></tr>)}</tbody></table></div>;
}
