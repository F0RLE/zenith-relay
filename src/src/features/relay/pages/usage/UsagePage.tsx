...
function AggregateView({ rows, field, empty }: { rows: UsageRow[]; field: "model" | "connection"; empty: string }) {
  ...
  return <div className="relay-table-wrap"><table className="relay-table usage-aggregate-table"><thead><tr><th>{field === "model" ? t("common.model") : t("usage.connection")}</th><th>{t("usage.requests")}</th><th>{t("common.success")}</th><th>{t("usage.inputTokens")}</th><th>{t("usage.cachedInputTokens")}</th><th>{t("usage.reasoningTokens")}</th><th>{t("usage.outputTokens")}</th><th>{t("usage.totalTokens")}</th><th>{t("usage.timing")}</th></tr></thead><tbody>{groups.map((group) => <tr key={group.name}><td><code>{group.name}</code></td><td>{group.requests}</td><td>{Math.round(group.success / group.requests * 100)}%</td><td>{group.inputTokens}</td><td>{group.cachedInputTokens}</td><td>{group.reasoningTokens}</td><td>{group.outputTokens}</td><td>{group.tokens}</td><td>{formatTiming(group.ttftCount ? Math.round(group.ttft / group.ttftCount) : null, Math.round(group.duration / group.requests))}</td></tr>)}</tbody></table></div>;
}
