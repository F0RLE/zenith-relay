...
export function UsagePage() {
  ...
  const filtered = rows.filter((item) => (status === "all" || (status === "success" ? item.success : !item.success)) && (!requestQuery.trim() || item.requestId?.toLocaleLowerCase().includes(requestQuery.trim().toLocaleLowerCase())));
  ...
  return (
    <section className="relay-page">
      ...
      {view === "requests" ? <RequestsView rows={filtered} status={status} setStatus={setStatus} requestQuery={requestQuery} setRequestQuery={setRequestQuery} formatTime={formatTime} onSelect={setSelected} /> : null}
