...
for (const width of [1160, 840, 390]) {
  for (const theme of ["light", "dark"] as const) {
    test(`model actions form one aligned group in ${theme} at ${width}px`, async ({ page }, testInfo) => {
      ...
      await installTauriMock(page, { locale: "ru", mode: "local", theme, populated: true, mixedModels: true, modelSpeed: { "gpt-5.4": "ultrafast" } });
      await page.goto("/");
      ...
      await expect(actions.locator(".model-protocol-button")).toBeVisible();
      ...
      expect(await actions.evaluate((cell) => {
        ...
        return buttons.every((button, i) => Math.abs(button.height - 34) < 1
      ...
      await actions.locator(".model-protocol-button").click();
