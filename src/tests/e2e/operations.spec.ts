...
test("local pool rotates requests by default and can explicitly enable chat affinity", async ({ page }) => {
  await installTauriMock(page, { mode: "local", locale: "en", populated: true });
  await page.goto("/");
  await page.getByRole("button", { name: "Pool", exact: true }).click();
  ...
  const calls = await page.evaluate(() => (window as unknown as { __TAURI_TEST_INVOKES__: Array<{ command: string; args: Record<string, unknown> }> }).__TAURI_TEST_INVOKES__);
  expect(calls.findLast((call) => call.command === "update_local_routing")?.args).toEqual({ input: { maxRetryCandidates: 5, sessionAffinity: true, sessionAffinityTtlSeconds: 300 } });
});
