...
import { formatSourceAmount, settledSourceStats, sourceStatsAmounts, sourceStatsStatus } from "../src/features/relay/sourceStatsModel";
...
describe("provider statistics presentation", () => {
  ...
  test("refresh failure retains successful values and marks them stale", () => {
    ...
    expect(settledSourceStats(legacy, failure)).toEqual({ value: legacy, loading: false, failed: true, error: "rate_limited" });
